...
func receive(ctx context.Context, event cloudevents.Event) {
    ...
    if err := markJobDone(resp.Name); err != nil {
        panic(err)
    }
}

func markJobDone(htcjobName string) error {
    ...
    defer db.Close()
    sqlStatement := `UPDATE htcjobs SET status=4 WHERE htcjobName=$1`
    _, err = db.Exec(sqlStatement, htcjobName)
    if err != nil {
        fmt.Printf("Error while inserting the job into DB")
        return err
    }
